...
def frames_to_video(frame_dir, video_path, fps=30):
    """Combine PNG frames in frame_dir into an MP4 video."""
    pngs = sorted(f for f in os.listdir(frame_dir) if f.endswith('.png'))
    if not pngs:
        print(f"  No PNG frames found in {frame_dir}")
        return
    with imageio.get_writer(video_path, fps=fps) as writer:
        for png in tqdm(pngs, desc=f"Writing {os.path.basename(video_path)}"):
            img = imageio.imread(os.path.join(frame_dir, png))
...
def render_circular_video(model_path, iteration, views, gaussians, pipeline, background, radius=0.5, n_frames=240, fps=30):
    ...
    for idx in range(n_frames):
        ...
        torchvision.utils.save_image(rendering, os.path.join(render_path, '{0:05d}'.format(idx) + ".png"))
    frames_to_video(render_path, os.path.join(model_path, 'circular', "ours_{}.mp4".format(iteration)), fps=fps)
...
def render_video(model_path, iteration, views, gaussians, pipeline, background, fps=30):
    ...
    for idx, pose in enumerate(tqdm(generate_ellipse_path(views,n_frames=600), desc="Rendering progress")):
        ...
        rendering = render(view, gaussians, pipeline, background)["render"]
        torchvision.utils.save_image(rendering, os.path.join(render_path, '{0:05d}'.format(idx) + ".png"))
    frames_to_video(render_path, os.path.join(model_path, 'video', "ours_{}.mp4".format(iteration)), fps=fps)
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, video: bool, circular:bool, radius: float, fps: int, args):
    with torch.no_grad():
        ...
        if circular:
            render_circular_video(dataset.model_path, scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, radius, fps=fps)
        ...
        if video:
            render_video(dataset.model_path, scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, fps=fps)
        #sample virtual view 
        if args.gaussians:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fps", default=30, type=int, help="Frames per second for output video")
    ...
    render_sets(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, args.video, args.circular, args.radius, args.fps, args)